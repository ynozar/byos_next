...
const LayoutToggle = React.forwardRef<HTMLDivElement, LayoutToggleProps>(
    ({ className, defaultLayout = "columns", onLayoutChange, ...props }, ref) => {
        ...
        return (
            <div
                ref={ref}
                className={cn("inline-flex items-center rounded-md border bg-muted p-1", className)}
                {...props}
            >
                <button
                    type="button"
...
const ScreenPreviewLayout = ({ children }: { children: React.ReactNode }) => {
    ...
    return (
        <div className="flex flex-col gap-4 items-start">
            ...
            <div className={cn("grid grid-cols-2 gap-4 w-[calc(800px*2+var(--spacing)*4)]", layout === "rows" && "grid-cols-1 w-[800px]")}>
                {children}
            </div>
        </div>
    )
